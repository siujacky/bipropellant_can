...
void MX_TIM_Init(void) {
  ...
  /* Enable TIM1 update interrupt so BLDC ISR fires at 16kHz.
   * Without this, DIER=0 → no ISR → MOE never cleared by BLDC controller
   * → all low-side FETs ON (complementary PWM with CCR=0) → phases shorted. */
