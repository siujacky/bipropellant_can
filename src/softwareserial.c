...
void SoftwareSerialInit(void){
  ...
  // setup TIM5: Changed from TIM2 to avoid conflict with control.c
  __HAL_RCC_TIM5_CLK_ENABLE();
  softwareserialtimer.Instance = TIM5;
  ...
  // Disable SWD/JTAG if using PA13/PA14 pins
  #ifdef SOFTWARE_SERIAL_USE_SWD_PINS
    __HAL_RCC_AFIO_CLK_ENABLE();
    __HAL_AFIO_REMAP_SWJ_DISABLE();  // Disable JTAG and SWD to release PA13/PA14/PA15/PB3/PB4
  #endif

  // Enable the appropriate EXTI interrupt based on pin number
  // The interrupt handler is determined at runtime in stm32f1xx_it.c
  uint16_t pin = SOFTWARE_SERIAL_RX_PIN;
  if (pin == GPIO_PIN_0) {
