...
#define BLDC_LIMITER_INCREMENT 3       // Symmetric-ish recovery (slower than limit to prevent oscillation)
#define BLDC_LIMITER_HYSTERESIS 50     // Don't increment until well below limit (prevents jitter)
...
void DMA1_Channel1_IRQHandler() {
  ...
  if (doLeft) {
    volatile MOTOR_ELECTRICAL *m = &electrical_measurements.motors[0];  // LEFT motor = index 0
    ...
    if(abs_dc > electrical_measurements.dc_adc_limit) {
      // Over current limit - reduce PWM quickly
      if (m->pwm_limiter > BLDC_LIMITER_DECREMENT) {
        m->pwm_limiter -= BLDC_LIMITER_DECREMENT;
      }
    } else if (abs_dc < (electrical_measurements.dc_adc_limit - BLDC_LIMITER_HYSTERESIS)) {
      // Well below limit - allow recovery (with hysteresis to prevent oscillation)
      if (m->pwm_limiter < 1024) {
        m->pwm_limiter += BLDC_LIMITER_INCREMENT;
        if (m->pwm_limiter > 1024) m->pwm_limiter = 1024;
      }
    }
    // else: in hysteresis zone - hold current value
