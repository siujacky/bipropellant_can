...
int setup_protocol(PROTOCOL_STAT *s) {
    ...
    main_ascii_init(s);
        ...
        errors += setParamVariable( s, 0xC0, UI_SHORT, &FlashContent.CAN_BoardID, sizeof(short) );
        setParamHandler( s, 0xC0, fn_FlashContentMagic );
        
        errors += setParamVariable( s, 0xC1, UI_SHORT, &FlashContent.CAN_BaseID_CMD, sizeof(short) );
        setParamHandler( s, 0xC1, fn_FlashContentMagic );
        
        errors += setParamVariable( s, 0xC2, UI_SHORT, &FlashContent.CAN_BaseID_STATUS, sizeof(short) );
        setParamHandler( s, 0xC2, fn_FlashContentMagic );
...
// Raw console output without protocol framing (for CAN debug messages)
void consoleLogRaw(char *message) {
    extern UART_HandleTypeDef huart2;
    extern UART_HandleTypeDef huart3;
    
    uint16_t len = 0;
    while(message[len] != '\0' && len < 512) len++;
    ...
    // Also output to Software Serial (PA13/PA14) if initialized
    #if defined(SOFTWARE_SERIAL) && defined(ENABLE_CAN_BUS)
    extern bool CAN_IsSoftwareSerialReady(void);
    if (CAN_IsSoftwareSerialReady()) {
        softwareserial_Send((unsigned char*)message, len);
    }
    #endif
}
