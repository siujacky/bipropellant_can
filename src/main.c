...
const FLASH_CONTENT FlashDefaults = {
  ...
  .CAN_BoardID = CAN_BOARD_ID,        // Default from config.h
...
#define WHEELBASE_MM 525.0
  ...
  // Try to detect MCP2515 and initialize CAN bus
  // Falls back to USART2/3 if MCP2515 not detected
  if (CAN_AutoDetectAndInit()) {
    CAN_PrintConfiguration();  // Print CAN IDs
  }
  #endif
  ...
  #ifdef ENABLE_CAN_BUS
  // Print CAN status after power button message
  if (can_mode_active) {
    forceLog("\r\n==============================================\r\n");
    forceLog("*** CAN CONTROLLER MODE ACTIVE ***\r\n");
    forceLog("==============================================\r\n\r\n");
    CAN_PrintConfiguration();
    
    // Send boot announcement over CAN bus
    for(volatile int i = 0; i < 500000; i++);  // ~50ms delay without HAL_Delay
    CAN_SendBootAnnouncement();
    
    forceLog("\r\n[INFO] CAN bus ready - listening for messages...\r\n");
    forceLog("[INFO] Statistics will print every 10 seconds\r\n\r\n");
  } else {
    forceLog("\r\n==============================================\r\n");
    forceLog("*** MCP2515 NOT DETECTED ***\r\n");
    forceLog("==============================================\r\n");
    forceLog("CAN mode disabled - check wiring:\r\n");
    forceLog("  CS:   PB12 (or your configured pin)\r\n");
    forceLog("  MOSI: PB15\r\n");
    forceLog("  MISO: PB14\r\n");
    forceLog("  SCK:  PB13\r\n");
    forceLog("  Check MCP2515 power (3.3V/5V) and 8MHz crystal\r\n\r\n");
  }
  #endif
...
//    }
    ...
    if ((debug_counter++) % 100 == 0) {
      ...
      #ifdef ENABLE_CAN_BUS
        // Send CAN status every 100ms
        CAN_SendStatus();
        // Check for CAN errors
        CAN_CheckErrors();
        
        // Print statistics every 10 seconds (10000 cycles)
        if (can_mode_active && (debug_counter % 10000) == 0) {
          CAN_PrintStatistics();
        }
      #endif
      consoleScope();
